...
class ProjectScreen(Screen):
    BINDINGS = [("r", "app.pop_screen", "Return"),
                ...
                ("c", "complete_project", "Complete Selected Project"),
    ...
    def on_mount(self) -> None:
        # get table
        self.results, self.headers = get_table(self.DB_FILE, "projects", self.condition)
        # Transform values
    ...
    def action_complete_project(self) -> None:
        ...
        datatable = self.query_one(DataTable)
        highlighted_project_row = datatable.cursor_row
        
        # Update the datatable
        coordinate_to_update = (highlighted_project_row, self.headers.index("status")) # Get coordinate of status column and highlighted row
        datatable.update_cell_at(coordinate_to_update, 2) # finish the project entry
    ...
    def action_add_project(self) -> None:
        ...
        with sqlite3.connect(self.DB_FILE) as conn:
            ...
            new_row[self.headers.index("name")] = "new project"
            ...
            # Add to DB
            cursor.execute(f"""INSERT INTO projects(id, name, begin_date, end_date, status) VALUES(?,?,?,?,?);""",
                           (new_row[self.headers.index("id")], new_row[self.headers.index("name")], 
                            ...
                            new_row[self.headers.index("status")]))
            # Commit changes
            conn.commit()   
    ...
    def update_db_at_cell(self, coordinate: tuple[int, int], new_value: str) -> None:
        ...
        with sqlite3.connect(self.DB_FILE) as conn:
            ...
            query = f"""UPDATE projects SET {header} = {new_value} WHERE id = {row_id};"""
            # Execute query
